...
function BracketCard({ matchId, data, onPick }) {
  ...
  function pick(team) {
    if (canPick) onPick(matchId, team.id);
  }
...
function BracketRound({ matchIds, matchupMap, onPick, rows, side }) {
  return (
    <div className={`bracket-round bracket-round--${rows} bracket-round--${side}`}>
      {matchIds.map(id => (
        <div key={id} className="bracket-slot">
          <BracketCard matchId={id} data={matchupMap[id]} onPick={onPick} />
        </div>
      ))}
    </div>
  );
    ...
    <div className="knockout-bracket">
      ...
      <div className="bracket-layout-wrap"><div className="bracket-layout">
        ...
        <BracketRound
          ...
          rows={8} side="left" matchupMap={matchupMap} onPick={onPick}
        />
        <BracketRound
          matchIds={['R16_1','R16_2','R16_3','R16_4']}
          rows={4} side="left" matchupMap={matchupMap} onPick={onPick}
        />
        <BracketRound
          matchIds={['QF_1','QF_2']}
          rows={2} side="left" matchupMap={matchupMap} onPick={onPick}
        />
        <BracketRound
          matchIds={['SF_1']}
          rows={1} side="left" matchupMap={matchupMap} onPick={onPick}
        />

        {/* Center: Final + Champion + Bronze */}
        <div className="bracket-center">
          <div className="bracket-center__final">
            <BracketCard matchId={FINAL_MATCHUP.id} data={finalData} onPick={onPick} />
          </div>
          {finalData?.winner && (
          ...
          <div className="bracket-center__bronze">
            ...
            <BracketCard matchId={THIRD_PLACE_MATCHUP.id} data={thirdData} onPick={onPick} />
          </div>
        </div>
        ...
        <BracketRound
          matchIds={['SF_2']}
          rows={1} side="right" matchupMap={matchupMap} onPick={onPick}
        />
        <BracketRound
          matchIds={['QF_3','QF_4']}
          rows={2} side="right" matchupMap={matchupMap} onPick={onPick}
        />
        <BracketRound
          matchIds={['R16_5','R16_6','R16_7','R16_8']}
          rows={4} side="right" matchupMap={matchupMap} onPick={onPick}
        />
        <BracketRound
          matchIds={['R32_9','R32_10','R32_11','R32_12','R32_13','R32_14','R32_15','R32_16']}
          rows={8} side="right" matchupMap={matchupMap} onPick={onPick}
        />
      </div></div>
